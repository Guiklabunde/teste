    ...
    <div className="w-full max-w-7xl mx-auto">
      ...
      {cart.length === 0 && (
        <div className="flex flex-col items-center justify-center">
          <p className="font-medium">
            Seu carrinho está vazio, aproveite os ótimos preços e compre alguns itens!
          </p>
      ...
      {cart.map((item) => (
        <section key={item.id} className="flex items-center justify-between border-b-2 border-gray-300">
          <img src={item.foto} alt={item.item} className="w-28" />

          <strong>Preço: {item.preco.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</strong>
      ...
      {cart.length !== 0 && (
        <p className="font-bold mt-4 float-right">
          Total: {total}
        </p>
      )}
    </div>
